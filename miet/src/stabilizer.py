...
class StabilizerState:
    ...
    def _g(self, x1, z1, x2, z2):
        ...
        mask10 = (x1 == 1) & (z1 == 0)
        result[mask10] = z2[mask10].astype(np.int32) * (
            2 * x2[mask10].astype(np.int32) - 1
        )
        mask01 = (x1 == 0) & (z1 == 1)
        result[mask01] = x2[mask01].astype(np.int32) * (
            1 - 2 * z2[mask01].astype(np.int32)
        )
        return result
    ...
    def measure(self, a, rng=None) -> int:
        ...
        if p is not None:
            ...
            self.tab[p, n + a] = 1
            outcome = int(
                rng.integers(2) if rng is not None else np.random.randint(0, 2)
            )
