...
def load_best():
    def filter_valid(L_vals, p_vals, mean, std, sem):
        ...
        idx = np.array(ok)
        return ([L_vals[i] for i in ok], p_vals,
                mean[idx], std[idx], sem[idx])
...
def check1_volume_law(L_vals, p_vals, mean_S, tee):
    ...
    tee.write(f"CHECK 1 (volume law):  alpha_vol={alpha:.3f}, R2={r2:.4f}, "
...
def check2_area_law(L_vals, p_vals, mean_S, tee):
    ...
    passed = sigma < 1.0
    tee.write(f"CHECK 2 (area law):    mean={mu:.2f}, std={sigma:.2f}, "
              f"{'PASS' if passed else 'FAIL'}")
    return passed
...
def check3_monotonicity(L_vals, p_vals, mean_S, tee):
    ...
    tee.write(f"CHECK 3 (monotonicity): slopes={slope_str}, "
              f"{'PASS' if passed else 'FAIL'}")
    return passed
...
def check4_crossing(L_vals, p_vals, mean_S, tee):
    ...
    tee.write(f"CHECK 4 (crossing):    p_cross={p_cross:.3f}, "
              f"{'PASS' if passed else 'FAIL'}  "
...
def check5_bell_entropy(tee):
    ...
    passed = (e == 1.0)
    tee.write(f"CHECK 5 (Bell entropy): S={e}, "
              f"{'PASS' if passed else 'FAIL'}"
              + ("" if passed else
                 "  -- entropy uses wrong subsystem columns"))
    return passed


def check6_warmup(tee):
    """
    ...
    def sample_mean_sem(warmup):
        vals = [single_sample(L, p, n_steps=4 * L, warmup=warmup)
                for _ in range(n_samples)]
        arr = np.array(vals)
        return float(arr.mean()), float(arr.std(ddof=1) / np.sqrt(n_samples))

    m2, e2 = sample_mean_sem(2 * L)
    m3, e3 = sample_mean_sem(3 * L)
    m5, e5 = sample_mean_sem(5 * L)

    combined_sem = np.sqrt(e2 ** 2 + e5 ** 2)
    diff         = abs(m2 - m5)
    passed       = diff < 2 * combined_sem

    tee.write(f"CHECK 6 (warmup):      "
              f"2L={m2:.3f}+/-{e2:.3f}, "
              f"3L={m3:.3f}+/-{e3:.3f}, "
              f"5L={m5:.3f}+/-{e5:.3f}, "
...
def check7_log_scaling(L_vals, p_vals, mean_S, tee):
    ...
    if not passed:
        if alpha < 0.05:
            note = ("  -> alpha ~ 0 or negative: check entropy formula "
                    "(must use B-complement columns, not A columns).")
        else:
            note = "  -> alpha > 4: check that p is near p_c, not in volume-law phase."

    tee.write(f"CHECK 7 (log scaling): alpha={alpha:.3f}, R2={r2:.4f} at p={p_used:.3f}, "
...
def main():
    ...
    tee.write(f"p range:     [{p_vals[0]:.3f}, {p_vals[-1]:.3f}]  ({len(p_vals)} points)")
    tee.write("")
