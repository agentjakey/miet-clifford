...
def main():
    ...
    # LEFT: raw S vs p with linestyles
    for i, L in enumerate(L_vals):
        c  = colors[i]
    ...
    # RIGHT: collapse plot -- markers + light connecting lines
    for i, L in enumerate(L_vals):
    ...
    ax_col.text(0.03, 0.97, ann,
                transform=ax_col.transAxes, fontsize=10, va="top",
    ...
    if n_L < 3:
        ...
        if pc_err > 0:
            print(f"\n  p_c deviation from Li et al.:  "
                  f"{abs(p_c_opt-0.16)/pc_err:.1f} sigma")
        if nu_err > 0:
            print(f"  nu  deviation from Li et al.:  "
                  f"{abs(nu_opt-1.3)/nu_err:.1f} sigma")
...
# ============================================================
# Finite-size scaling collapse under the ansatz
# $S(L,p) = f\!\left((p-p_c)\cdot L^{1/\nu}\right)$.
# (Left) Raw $S(L/2)$ vs.\ $p$ for $L \in \{8, 12, 16, 20, 24\}$;
# shaded bands are $\pm 1$ SEM. Curves are distinguished by color and linestyle.
# (Right) Collapsed curves with optimized parameters
# $p_c = 0.209 \pm 0.011$, $\nu = 1.72 \pm 0.15$
# (Nelder-Mead; bootstrap uncertainty, 100 resamples).
# Literature values $p_c \approx 0.16$, $\nu \approx 1.28$~\cite{Zabalo2020}
# are shown in the inset; residual deviations reflect finite-size corrections
# at $L \leq 24$.
