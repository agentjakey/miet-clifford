...
OUT_PDF = os.path.join(
    os.path.dirname(__file__), "../figures/fig3_scaling_collapse.pdf"
)
OUT_PNG = os.path.join(
    os.path.dirname(__file__), "../figures/fig3_scaling_collapse.png"
)
...
def _collapse_cost(params, L_vals, p_vals, mean_S, alpha_eff=0.0, p_lo=None, p_hi=None):
...
def _optimize(
    L_vals, p_vals, mean_S, p_c0=0.16, nu0=1.3, alpha_eff=0.0, p_lo=None, p_hi=None
):
    bounds_pc = (0.08, 0.28)
    bounds_nu = (0.5, 3.0)
    ...
    def cost_bounded(params):
        ...
        if not (bounds_nu[0] <= nu <= bounds_nu[1]):
            return 1e9
        return _collapse_cost(
            params, L_vals, p_vals, mean_S, alpha_eff=alpha_eff, p_lo=p_lo, p_hi=p_hi
        )
...
def _sensitivity_analysis(L_vals, p_vals, mean_S, pc0, nu0):
    ...
    def _fit(L_sub, p_lo=None, p_hi=None, label="", p_window="full"):
        ...
        try:
            pc, nu = _optimize(
                L_sub, p_vals, m_sub, p_c0=pc0, nu0=nu0, p_lo=p_lo, p_hi=p_hi
            )
        except Exception:
...
def _format_sensitivity_table(rows, pc_main, nu_main):
    ...
    lines.append("=" * 70)
    lines.append(
        f"{'Variant':<32} {'L used':<16} {'p window':<18} "
        f"{'pc_eff':>8} {'nu_eff':>7}"
    )
    lines.append("-" * 70)
    for r in rows:
    ...
    if valid_pc:
        lines.append(
            f"  pc_eff range: [{min(valid_pc):.4f}, {max(valid_pc):.4f}]  "
            f"(spread = {max(valid_pc)-min(valid_pc):.4f})"
        )
    if valid_nu:
        lines.append(
            f"  nu_eff range: [{min(valid_nu):.4f}, {max(valid_nu):.4f}]  "
...
def main():
    ...
    print(
        "  The simplified ansatz omits the critical log term; collapse is diagnostic."
    )

    print("Bootstrap uncertainty (100 resamples) ...")
    pc_boot, nu_boot = _bootstrap(
        L_vals, p_vals, mean_S, sem_S, n_boot=100, p_c0=pc_eff, nu0=nu_eff
    )
    ...
    sens_rows = _sensitivity_analysis(L_vals, p_vals, mean_S, pc0=pc_eff, nu0=nu_eff)
    ...
    print(
        f"  pc_eff = {pc_eff:.4f} +/- {pc_err:.4f}  "
        f"(stat. only; Li: 0.160, Zabalo: 0.160)"
    )
    print(
        f"  nu_eff = {nu_eff:.4f} +/- {nu_err:.4f}  "
        f"(stat. only; Li: 1.3,   Zabalo: 1.28)"
    )
    print(
        "  Deviations from literature are expected systematic finite-size "
