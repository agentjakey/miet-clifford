...
def _load_best():
    def filter_valid(L_vals, p_vals, mean, std, sem):
        valid = [i for i, L in enumerate(L_vals) if not np.any(np.isnan(mean[i]))]
        if not valid:
            return None
        idx = np.array(valid)
        return ([L_vals[i] for i in valid], p_vals, mean[idx], std[idx], sem[idx])
...
def _bootstrap_crossing_uncertainty(
    p_vals, s1, s2, e1, e2, n_boot=N_BOOT_CROSS, rng_seed=0
):
...
def _compute_crossing_table(L_vals, p_vals, mean_S, sem_S):
    ...
    for i in range(len(L_vals) - 1):
        ...
        if pc is not None:
            unc = _bootstrap_crossing_uncertainty(p_vals, s1, s2, e1, e2, rng_seed=i)
        else:
            unc = float("nan")
...
def _format_crossing_table(rows, mean_cross, largest_cross, largest_unc, spread):
    ...
    lines.append(
        "Spread across pairs = systematic finite-size drift (NOT stat. error)."
    )
    lines.append("=" * 72)
    lines.append(
        f"{'L1':>5}  {'L2':>5}  {'pcross_eff':>12}  " f"{'boot_unc':>10}  Note"
    )
    lines.append("-" * 72)
    for r in rows:
        pc_str = f"{r['pcross_eff']:.4f}" if r["pcross_eff"] is not None else "  none "
        un_str = (
            f"{r['uncertainty']:.4f}" if np.isfinite(r["uncertainty"]) else "  n/a  "
        )
        note = r["note"] or ""
    ...
    lines.append(
        f"  Largest-L crossing ({rows[-1]['L1']} x {rows[-1]['L2']}):"
...
def main():
    ...
    valid_crosses = [r["pcross_eff"] for r in cross_rows if r["pcross_eff"] is not None]
    ...
    spread = (
        (max(valid_crosses) - min(valid_crosses)) if len(valid_crosses) >= 2 else None
    )
    ...
    cross_json = {
        ...
        "spread": spread,
        "note": (
            "Spread reflects systematic finite-size drift; "
    ...
    ax.axvline(
        ...
        linestyle="--",
        linewidth=1.4,
        label=r"$p_c^{\mathrm{lit}} \approx 0.16$",
        zorder=4,
    )
    if mean_cross is not None:
        ax.axvline(
    ...
    ax.set_title(
        "Finite-Size Signatures of the MIPT\n"
    ...
    for r in cross_rows:
        ...
        print(
            f"  L={r['L1']:>2} x L={r['L2']:>2}:  pcross_eff = {pc_str} {un_str}"
    ...
    print(
        "  Upward drift of pcross_eff is expected and well-documented " "for L <= 50."
    )
